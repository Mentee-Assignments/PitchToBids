...
file_dict['flankers'] = glob(path + prefix + '/Flanker/run[1-2]/Flanker[1-2]_RPI.nii.gz')
...
for scantype, filenames in file_dict.items():
    if scantype == 't1s':
        ...
        repl = r"sub-0\1/ses-\2\3/func/sub-0\1_ses-\2\3_task-flanker_run-0\4_bold.nii.gz"
    ...
    for filename in filenames:
        ...
        if res is None:
            print('ERROR no file for {file}'.format(file=filename))
        else:
